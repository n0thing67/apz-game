...
# Fallback: если в конкретной версии MAX user_id не приходит/не парсится,
# ведём состояние регистрации по chat_id (он всегда числовой и стабилен в рамках диалога).
REG_STATE_CHAT: Dict[int, str] = {}              # chat_id -> "waiting_fullname" | "waiting_age"
REG_NAME_CHAT: Dict[int, Tuple[str, str]] = {}   # chat_id -> (first_name, last_name)
...
async def _send_welcome(bot: Bot, chat_id: int, user_id: int) -> None:
    ...
    if user_id:
        ...
        # user_id может отсутствовать в BotStarted, а иногда и не парситься из MessageCreated.
        # Поэтому ставим fallback-состояние по chat_id, чтобы регистрация не "замирала".
        REG_STATE_CHAT[chat_id] = "waiting_fullname"
        REG_NAME_CHAT.pop(chat_id, None)
    ...
    async def on_any_message(event: MessageCreated):
        ...
        if _is_start_request(text):
            if user_id:
                ...
                REG_NAME_UID.pop(user_id, None)
            if chat_id:
                REG_STATE_CHAT.pop(chat_id, None)
                REG_NAME_CHAT.pop(chat_id, None)
            await _send_welcome(event.bot, chat_id, user_id)
            return
        ...
        if _is_stats_request(text):
            ...
            return
        # --- Регистрация ---
        # В MAX иногда user_id может отсутствовать/не парситься в зависимости от версии клиента/библиотеки.
        # Чтобы регистрация не "замирала", используем reg_key = user_id (если есть) иначе chat_id.
        reg_key = user_id or chat_id
        if not reg_key:
            return
        # 1) Если пользователь уже зарегистрирован — можно игнорировать «шум».
        user = await get_user(reg_key)
        if user:
            return
        # 2) Берём состояние регистрации.
        state = REG_STATE_UID.get(user_id) if user_id else REG_STATE_CHAT.get(chat_id)
        ...
        if not state:
            ...
            if parsed:
                if user_id:
                    ...
                    REG_NAME_CHAT[chat_id] = parsed
                    REG_STATE_CHAT[chat_id] = "waiting_age"
        ...
        if state == "waiting_age":
            ...
            if user_id:
                ...
                first_name, last_name = REG_NAME_CHAT.get(chat_id, ("", ""))
            try:
                await register_user(reg_key, first_name, last_name, age)
            except Exception:
                pass
            if user_id:
                REG_STATE_UID.pop(user_id, None)
